...
### 导出：纯文本 .txt
- 格式：**每行一条 `单词|释义`**，与内置词库 `wordbooks.js` 的 `data` 字段同构；
  文件头部加 `\ufeff`（BOM），Windows 记事本 / Excel 打开中文不乱码。
- 编码：`new Blob([...], { type: 'text/plain;charset=utf-8' })`；文件名 `白纸名-YYYY-MM-DD.txt`。
- 序列化唯一入口：`serializeWords(list)` —— 以后要改格式只动这一处。

### 导入：txt 与旧 json 双兼容
- 解析唯一入口：`parseImportText(raw, startIndex)`，按首个非空字符分流：
  - `[` / `{` 开头 → 按旧版 JSON 备份解析，`x / y / recallCount` 等字段原样保留（向下兼容）；
  - 其余 → 按文本解析，**分隔符支持 `|`、全角 `｜`、制表符 `\t`**（取最靠前的一个），
    跳过空行与 `#` 注释，**无分隔符的行按「只有单词、释义为空」保留**（不丢弃）。
- 导入是**追加**到当前白纸，不覆盖；坐标由 `gridSpot(i)` 递补网格位，
  同一张纸连续导入多次也不会叠在一点。
- `#import-file` 的 `accept` 必须同时含 `.txt` 与 `.json`，否则手机文件选择器会藏掉 txt。

## 单词删除（v1.5.0）

- 唯一入口 `removeWord(id)`：二次确认 → 原地 `splice`（保持对 `activePaper.words` 的引用）
  → `saveWords()` → 若屏上那张卡就是它则 `dismissWord()` → 统计面板开着就 `renderStats()` → toast。
- 两个调用点：统计面板每行的 `.stat-del`、记忆模式单词卡的 `.word-del`（与 `.word-speak` 左右对称）。
- **新增删除入口时务必走 `removeWord()`**，不要各自写 splice，否则计数 / 面板 / 屏上卡片会不同步。

## 移动端断点（v1.5.0）

- 断点 **600px**（原来是 480px，覆盖不到 iPhone Pro Max 的 430pt 与部分安卓机）。
- 工具栏在 600px 以下：`flex-wrap` 两行 —— 第一行标题，第二行 `.controls` 整块；
  `.controls` 改为 **4 列等宽网格** `repeat(4, minmax(0, 1fr))`（`1fr` 的自动最小值会让列宽参差，
  必须写 `minmax(0, 1fr)`）；`.mode-switch` 用 `grid-column: 1 / -1` 独占一行三等分。
- 触摸目标 ≥ 44px，间距 8px。新增按钮时必须保住这条。
- **游戏进行中**：JS 在 `setMode()` 里给 `body` 加 `game-mode`，CSS 据此收起标题行与次要按钮，
  工具栏只剩模式切换；此时 `.game-hud` 落在 80px、`.game-combo` 落在 160px（HUD 底边 148px）。
  **改这两个数值前先量一下工具栏高度**，否则 HUD 会压在工具栏或连击环上。

## 游戏释义框的定位约束（v1.5.0）

- `.def-pop` 必须 `width: max-content`：绝对定位盒子的可用宽度 = 容器宽 − `left`，
  单词贴屏幕右缘时可用宽度只剩几十像素，收缩式布局会把盒子压成窄条 → 释义被挤成多行。
- `showDefinition(cx, cy, word)` 追加 DOM 后**测量 `offsetWidth/offsetHeight` 再回写 left/top**，
  把中心点收进可视区（四周各留 10px）。这是修「边缘变形」的关键两步，缺一不可。
...
- □ 
  ...
  夜间模式下滚动页面 / 统计面板 / 白纸夹横向列表，滚动条为浅米色，不再出现刺眼的白色系统滚动条。

- □ 

  导出得到 `.txt`，用记事本打开是「单词|释义」逐行、中文不乱码；再导入同一文件，单词与释义一一对应。

- □ 

  导入旧的 `.json` 备份仍可用；导入 `.txt` 时制表符 / 全角竖线 / 空行 / `#` 注释都能正确跳过。

- □ 

  统计面板每行可删除该单词；记忆模式单词卡左上角可删除；确认框选取消时不删除，删除后计数与列表同步刷新。

- □ 

  游戏里把单词逼到屏幕最左 / 最右切开，释义框整体留在屏幕内、不被压成窄条、文字不多行变形。

- □ 

  手机（≤600px）打开：工具栏为「标题行 + 操作区」两行，模式切换独占一行，
  所有按钮高度 ≥44px、间距 8px，不出现横向溢出；游戏进行中工具栏只剩模式切换。

## 版本记录

### v1.5.0（2026-09-12）· 五处体验问题修复
- **夜间滚动条**：新增 `--scroll-thumb` / `--scroll-thumb-hover` 主题变量 + 全套滚动条样式
  （`scrollbar-color` 与 `::-webkit-scrollbar-*` 两套），夜间模式滚动条不再亮出白杠。
- **导出改 txt**：由 `.json` 改为 UTF-8 带 BOM 的 `.txt`，每行「单词|释义」；导入同时兼容新 txt 与旧 json
  （`parseImportText()` 分流），旧备份的 `x / y / recallCount` 仍原样保留。
- **新增删除按钮**：统计面板每行 + 记忆模式单词卡，共用 `removeWord(id)`（确认 → splice → 落盘 → 同步刷新）。
- **游戏释义框**：`.def-pop` 改 `width: max-content`，`showDefinition()` 测量后把中心点收进可视区 ——
  修掉「单词抛在边缘、切开后释义框变形、文字挤成多行」。
- **移动端排版**：断点 480 → **600px**；工具栏拆两行、操作区 4 列等宽网格、触摸目标 ≥ 44px；
  游戏进行中 `body.game-mode` 收起次要按钮，HUD/连击环顺势下移；首页按钮、卡片按钮、统计删除键一并放大。
  清掉原来 `gap: 4px` / 按钮高约 26px 的挤压写法。
- **测试**：新增 `tests/word-io.test.js` 12 条；全量 **51 条全绿**（game-mode 31 + wordbook 8 + word-io 12）。
  其中「释义框贴边不越界」用例在旧实现上实测失败（left=1272 > 视口），确认是有效回归测试。
